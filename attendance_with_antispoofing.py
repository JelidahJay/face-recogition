...
def login():
            ...
            if row == None:
                ...
                def manage_employee():
                    try:
                        ...
                        background_pic.pack()
                        first.title("Manage Information post")
                        print("Hi Chhabi lal tamang")
                        face = Label(first, text = "Management of Information" , bg = "green" , fg = "yellow", padx = 15, pady = 15, font = ("Times New Roman", 20, "bold") ,borderwidth = 5, relief = RIDGE).place(x = 500, y = 10)
                                        ...
                                        if (re.search('^[9]\d{9}$', contact_var.get())):
                                            ...
                                            if(re.search(regex, address_var.get())):
                                                ...
                                                if not os.path.exists(input_directory):
                                                    ...
                                                    conn.commit()
                                                    cur3 = conn.cursor()
                                                    cur3.execute("insert into barcode_data(ID,name,doj)VALUES(%s,%s,%s)",(contact_var.get(),fname_var.get(),DOJ_var))
                                                    conn.commit()
                                                    cur2 = conn.cursor()
                            ...
                            if post_var.get() == "" or fname_var.get() == "" or gender_var.get() ==  "" or contact_var.get() == "" or address_var.get() == "":
                                ...
                                if (re.search('[a-zA-Z]+', fname_var.get())):
                                    if len(contact_var.get()) != 10:
                                        ...
                                        if(re.search('^[2]\d{9}$', contact_var.get())):
                                            regex = '^(\w|\.|\_|\-)+[@](\w|\_|\-|\.)+[.]\w{2,3}$'
                                            if(re.search(regex, address_var.get())):
                                                id = eid_var.get()
                                                name = fname_var.get()
                    ...
                    ################################################## Employee Management form ###############################
                        ...
                        titles = Label(f2, text = "Manage Information" ,bg = "gray", font = ("Italic", 20, "bold")).place(x = 90, y = 30)
                        id = Label(f2, text = "ID", bg = "gray", font = ("italic",13, "bold")).place(x = 35, y = 100 )
                        ...
                        no = Label(f2, text = "Id No", bg = "gray", font = ("italic",12, "bold")  ).place(x = 35, y = 300)
    ...
    ######################################### Function to recognize the face
                ...
                def face_recognize():
                    ...
                    if os.path.exists(embeddings_model_file and recognizer_model_file): 
                        ...
                        if final_label== "real":
                            ...
                            start_hour = 0
                            end_hour = 24
                            status = "Appoved Access"
                            cur1 = conn.cursor()
                            cur1.execute("select barcode from barcode_logs where name = %s",(final_name))
                            data = cur1.fetchall()
                            if len(data)== 0:
                                messagebox.showwarning("Warning","Sorry {}.Barcode and face do not match".format(final_name))     
                            else:
                ...
                B1 = Button(attendance, image = photo1, text = "Information Management",font = ("Times New Roman" , 15), fg = "green", height =230, width = 265, command = manage_employee, compound = BOTTOM )
                B1.place(x = 20, y = 100)

                photo2 = PhotoImage(file = "Photos/face_recognizer.png",  master = attendance)
                B2 = Button(attendance, image = photo2 , text = "Face Recognizer", font = ("Times new roman", 15), fg = "green", height = 230, width= 265, command = barcode_lock, compound = BOTTOM )
                B2.place(x = 20, y = 400)
